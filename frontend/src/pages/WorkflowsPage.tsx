...
const createStartNode = (
	position: Position = DEFAULT_START_POSITION
): WorkflowNode => ({
	id: START_NODE_ID,
	type: 'start',
	data: { label: 'Start' },
...
const mapAgentResponse = (
	agent: AgentNodeResponse,
	index: number
): WorkflowNode => ({
	id: agent.id,
	type: 'agent',
	data: {
		...
		pathCount: 0,
		llmModel:
			typeof agent.llm_config === 'object'
...
const extractTransferMessage = (
	metadata?: Record<string, unknown> | null
): string | undefined => {
	if (
		metadata &&
		typeof metadata === 'object' &&
		'transferMessage' in metadata
	) {
...
export const WorkflowsPage: React.FC = () => {
	...
	const updateAgentMetadata = useCallback(
		async (
			...
			const currentClean = Object.entries(currentMetadataRaw).reduce<
				Record<string, unknown>
			>((acc, [key, value]) => {
				if (value !== undefined && value !== null) {
					acc[key] = value;
			...
			try {
				await apiUpdateAgent(agentId, {
					...
					llm_config: data.llmModel
						? { provider: 'openai', model: data.llmModel as string }
						: undefined,
			...
			const summaries = results.map((workflow) => {
				const summary: WorkflowSummary = {
					...
					latestPublishedVersionId:
						workflow.latest_published_version_id ?? undefined,
	...
	const selectAgentById = useCallback(
		(nodeId: string) => {
			...
			setSelectedAgent({
				...
				instructions: (node.data as Record<string, unknown>)
					.instructions as string,
				toolCount: (node.data as Record<string, unknown>).toolCount as number,
				tools: (node.data as Record<string, unknown>).tools as
					| ToolOperation[]
					| undefined,
				toolIds: (node.data as Record<string, unknown>).toolIds as
					| string[]
					| undefined,
				pathCount: paths.length,
				paths,
				model:
					((node.data as Record<string, unknown>).llmModel as
						| string
						| undefined) ?? undefined,
				metadata: (node.data as Record<string, unknown>).metadata as Record<
					string,
					unknown
				> | null,
				position: node.position,
	...
	const transformConfigToCanvas = useCallback(
		(config: WorkflowConfigResponse) => {
			const startPosition = parsePosition(
				config.start_position ?? null,
			...
			config.agents.forEach((agent, index) => {
				...
				const toolResponses = (config.tools[agent.id] ??
					[]) as AgentToolResponse[];
				const parsedTools = toolResponses.map((tool) =>
					parseToolResponse(tool)
				);
				...
				const mappedPaths: Path[] = paths.map((path) => {
					const variables = (
						config.path_variables[path.id] ?? []
					).map<PathVariable>((variable) => ({
						id: variable.id,
						name: variable.name,
					...
					if (!hideEdge) {
						newEdges.push({
							id: `edge-${path.id}`,
							source: path.from_agent_id,
							target: path.to_agent_id,
						});
					}
					return mapped;
				});
				...
				if (
					...
					(agent.metadata as Record<string, unknown>).is_entry
				) {
	...
	const handleCreateWorkflow = useCallback(async () => {
		...
		try {
			...
			const summary: WorkflowSummary = {
				...
				latestPublishedVersionId:
					created.version.status === 'published'
						? created.version.id
						: undefined,
			...
			setWorkflows((prev) => [
				summary,
				...prev.filter((wf) => wf.id !== summary.id),
			]);
	...
	const ensureStartEdge = useCallback(
		(targetId: string | null) => {
			setEdges((prev) => {
				const withoutStart = prev.filter(
				...
				return [
					...withoutStart,
					{ id: 'start-edge', source: START_NODE_ID, target: targetId },
				];
			});
		},
		[setEdges]
	...
	const syncAgentTools = useCallback(
		async (
			...
			try {
				...
				const newOpsMap = new Map<string, ToolOperation>(
					newToolOps.map((op) => [op.id, op])
				);
					...
					for (let i = 0; i < existingToolOps.length; i++) {
						...
						if (newOpsMap.has(existingOp.id)) {
							...
							if (incoming) {
								await apiUpdateTool(
									backendId,
									toolOperationToPayload(incoming)
								);
			...
			try {
				...
				if (updatedAgent.isNew) {
					...
					if (updatedAgent.tools && updatedAgent.tools.length > 0) {
						const result = await syncAgentTools(
							response.id,
							updatedAgent.tools
						);
					...
					let readyConnection = null as PendingEdge | null;
					setPendingEdges((prev) => {
						const updated = prev
							.map((edge) => {
								const sourceId =
									edge.sourceId === tempId ? response.id : edge.sourceId;
								const targetId =
									edge.targetId && edge.targetId === tempId
										? response.id
										: edge.targetId;
					...
					// Sync tools with backend
					const existingToolIds = (node.data as Record<string, unknown>)
						.toolIds as string[] | undefined;
					const existingTools = (node.data as Record<string, unknown>).tools as
						| Array<string | ToolOperation>
						| undefined;
					const {
						operations: syncedToolsExisting,
	...
	const handleSavePath = useCallback(
		async (pathPayload: PathSheetPayload): Promise<boolean> => {
			...
			if (
				!pathPayload.targetAgentId ||
				pathPayload.targetAgentId.startsWith('temp-')
			) {
			...
			try {
				...
				const mappedPath: Path = {
					...
					transferMessage: extractTransferMessage(
						createdPath.metadata ?? undefined
					),
				...
				if (connectionDraft) {
					...
					setPendingEdges((prev) =>
						prev.filter((edge) => edge.edgeId !== connectionDraft.edgeId)
					);
				...
				if (connectionDraft) {
					setEdges((prev) =>
						prev.filter((edge) => edge.id !== connectionDraft.edgeId)
					);
					setPendingEdges((prev) =>
						prev.filter((edge) => edge.edgeId !== connectionDraft.edgeId)
					);
	...
	const handleDeletePath = useCallback(
		async (pathId: string | number) => {
			...
			try {
				...
				setAgentPaths((prev) => ({
					...prev,
					[selectedNodeId]: (prev[selectedNodeId] ?? []).filter(
						(path) => path.id !== id
					),
				}));
	...
	const assignEntryAgent = useCallback(
		async (agentId: string) => {
			...
			const added = await updateAgentMetadata(agentId, (metadata) => ({
				...metadata,
				is_entry: true,
			}));
			if (!added) {
				if (previousId && !previousId.startsWith('temp-')) {
					void updateAgentMetadata(previousId, (metadata) => ({
						...metadata,
						is_entry: true,
					}));
				}
	...
	const handleConnect = useCallback(
		(connection: Connection) => {
			...
			const sourceIsNew = Boolean(
				(sourceNode.data as Record<string, unknown>)?.isNew
			);
			...
			if (sourceIsNew || targetIsNew || !targetIsAgent) {
				...
				setPendingEdges((prev) => {
					const exists = prev.some(
						(edge) =>
							edge.sourceId === connection.source && edge.targetId === targetId
					);
					if (exists) {
						return prev;
					}
	...
	const persistAgentPosition = useCallback(
		async (agentId: string, position: Position) => {
			if (agentId.startsWith('temp-')) {
				return;
			}
			const node = nodes.find((item) => item.id === agentId);
			...
			try {
				const metadataValue = data.metadata as
					| Record<string, unknown>
					| null
					| undefined;
				await apiUpdateAgent(agentId, {
	...
	const persistStartPosition = useCallback(
		async (position: Position) => {
			...
			try {
				await updateWorkflowVersionConfig(currentVersionId, {
	...
	const handleNodesChange = useCallback(
		(changes: NodeChange[]) => {
			...
			if (removedIdsSet.size > 0) {
				...
				let updatedAgentPaths = null as Record<string, Path[]> | null;
				setAgentPaths((prev) => {
					...
					for (const [agentId, paths] of Object.entries(prev)) {
						...
						next[agentId] = paths.filter(
							(path) => !removedIds.includes(path.targetAgentId)
						);
					}
				...
				setConnectionDraft((prev) => {
					...
					if (
						removedIds.includes(prev.sourceId) ||
						removedIds.includes(prev.targetId)
					) {
						return null;
					}
					return prev;
			...
			setCurrentWorkflow((prev) =>
				prev
					? {
							...
							latestDraftVersionId:
								prev.latestDraftVersionId === published.id
									? null
									: prev.latestDraftVersionId,
					  }
					: prev
			);
		...
		<>
			<div className="flex-1 flex flex-col">
				...
				{currentView === 'list' ? (
					<div className="flex-1 flex flex-col">
						{workflowError && (
							<div className="p-4 text-sm text-error">{workflowError}</div>
						)}
						<WorkflowsList
							onCreateWorkflow={() => {
					...
					<div className="flex flex-1 overflow-hidden">
						<div className="flex-1 flex flex-col">
							{canvasError && (
								<div className="p-4 text-sm text-error">{canvasError}</div>
							)}
			...
			<TestChatModal
				...
				workflowName={currentWorkflow?.name}
				workflowVersionId={
					showTestModal && currentVersionId ? currentVersionId : null
				}
			/>
		</>
	);
