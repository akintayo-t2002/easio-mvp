...
import { INTEGRATIONS, getIntegration, type IntegrationDefinition, type OperationDefinition } from "../config/operations"
import { useIntegrationStatus } from "../hooks/useIntegrationStatus"
import { useNavigate } from "react-router-dom"
import { buildIntegrationAuthorizeUrls } from "../lib/utils"

const TEMPLATE_PROMPTS = ["Customer support specialist", "Appointment specialist", "Lead qualification specialist"]
...
type ModalState =
  ...
  | { type: "operation-config"; integration: IntegrationDefinition; operation: OperationDefinition; existingConfig?: ToolOperation }
...
export default function EditAgentModal({ agent, onClose, onSave }: EditAgentModalProps) {
  ...
  const oauthWindowRef = useRef<{ win: Window | null; provider: string | null }>({ win: null, provider: null })
  ...
  const handleOAuthMessage = useCallback(
    (event: MessageEvent) => {
      ...
      if (messageType !== "integration_oauth_complete" && messageType !== "airtable_oauth_complete") return
      ...
      const errorMessage = typeof data.error === "string" && data.error.length > 0 ? data.error : undefined
  ...
  useEffect(() => {
    ...
    closeMonitorRef.current = window.setInterval(() => {
      ...
      if (currentWindow && currentWindow.closed) {
        ...
        setConnectErrors((prev) => ({
          ...prev,
          [provider]: prev[provider] ?? `${provider} connection window was closed before completion.`,
        }))
      }
    }, 500)
  ...
  const handleConnect = (integrationId: string) => {
    ...
    const { authorizeUrl, callbackUrl } = buildIntegrationAuthorizeUrls({ provider: integrationId })
  ...
  const currentOperationProvider = modalState.type === "operation-selector" ? modalState.integration.id : null
  const currentIntegrationConnected = currentOperationProvider === "airtable"
          ...
          <Dialog.Content className="fixed inset-0 z-[61] flex items-center justify-center px-4 py-8">
            ...
            <div className="flex flex-1 overflow-hidden">
              ...
              <div className="flex-1 space-y-6 overflow-y-auto border-r border-border p-6">
                {/* Agent Name */}
                <div>
                  <label className="text-xs font-semibold uppercase text-text-secondary">Agent Name</label>
                  <Input
                    value={agentName}
                ...
                <div>
                  <div className="mb-2 flex items-center justify-between">
                    <label className="text-xs font-semibold uppercase text-text-secondary">Instructions</label>
                    <button className="rounded p-1 transition-colors hover:bg-background-secondary">
              ...
              <div className="flex w-2/5 flex-col space-y-6 overflow-y-auto bg-background-secondary/60 p-6">
                {/* Model Selector */}
                <div>
                  <label className="text-xs font-semibold uppercase text-text-secondary">Model</label>
                  <select
                    value={model}
                ...
                <div>
                  <div className="mb-3 flex items-center justify-between">
                    <label className="text-xs font-semibold uppercase text-text-secondary">Tools</label>
                    <Popover.Root open={toolsPopoverOpen} onOpenChange={setToolsPopoverOpen}>
                      ...
                      <Popover.Portal>
                        ...
                        >
                          <div className="mb-2 px-2 text-xs font-semibold text-text-secondary">Select Integration</div>
                            ...
                            {Object.values(INTEGRATIONS).map((integration) => {
                              ...
                              return (
                                ...
                                >
                                  ...
                                  <span className="flex-1 text-sm font-medium text-text-primary">{integration.name}</span>
                                </button>
                              )
                            })}
                          </div>
                    ...
                    <div className="flex flex-wrap gap-2">
                      {toolOperations.map((toolOp) => {
                        ...
                        return (
                          ...
                          >
                            <Icon className={`h-4 w-4 ${integration.color}`} />
                            <span className="text-xs font-medium text-text-primary">{toolOp.operationName}</span>
                            <button
                ...
                <div className="space-y-2 border-t border-border pt-4">
                  <label className="flex items-center gap-2 text-sm text-text-primary">
                    <input type="checkbox" className="rounded border-border text-accent focus:ring-offset-0" defaultChecked />
                    Knowledge Base
                  </label>
                  <label className="flex items-center gap-2 text-sm text-text-primary">
                    <input type="checkbox" className="rounded border-border text-accent focus:ring-offset-0" />
                    Buttons
                  </label>
                  <label className="flex items-center gap-2 text-sm text-text-primary">
                    <input type="checkbox" className="rounded border-border text-accent focus:ring-offset-0" />
            ...
            <div className="flex h-16 items-center justify-end gap-3 border-t border-border px-6 backdrop-blur">
              <Button variant="outline" onClick={onClose} className="border-border text-text-primary">
                Cancel
              </Button>
              <Button
                onClick={() => {
