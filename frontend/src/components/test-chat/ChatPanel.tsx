...
export const ChatPanel: React.FC<ChatPanelProps> = ({
  ...
  const messages = useMemo(() => {
    ...
    const merged: ChatEntry[] = [...transcriptEntries.values()].filter(
      (entry) => !entry.interim || entry.isAgent
    );
  ...
  useEffect(() => {
    ...
    void room.localParticipant
      .publishData(
        JSON.stringify({ type: 'ready_to_listen' }),
        //new TextEncoder().encode(JSON.stringify({ type: 'ready_to_listen' })),
        { reliable: true }
      )
      .catch((error: unknown) => {
  ...
  return (
    <div className="flex h-full flex-col bg-white">
      ...
      <div ref={containerRef} className="flex-1 overflow-y-auto px-6 py-4 space-y-4">
        {messages
          ...
          .map((entry) => (
            <div
              key={entry.id}
              className={`flex flex-col ${entry.isSelf ? 'items-end' : 'items-start'}`}
            >
              <span
                className={`text-xs font-semibold ${
                  entry.isSelf ? 'text-text-secondary' : accentColorClass
                }`}
              >
                {entry.name}
              </span>
        ...
        {((!hasAgentContent && !isConnecting) || isAgentThinking || isConnecting) && (
          <TypingIndicator />
        )}
      </div>
      ...
      <form
        ...
        className="border-t border-border bg-background-secondary/60 px-6 py-4"
      >
