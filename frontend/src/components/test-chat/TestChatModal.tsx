...
export function TestChatModal({ open, onClose, workflowName, workflowVersionId }: TestChatModalProps) {
  const { status, session, shouldConnect, error, start, stop } = useTestSession()
        ...
        <Dialog.Content className="fixed inset-0 z-50 flex items-center justify-center px-4 py-8">
          <div className="flex h-full max-h-[85vh] w-full max-w-4xl flex-col overflow-hidden rounded-2xl bg-white shadow-2xl">
            <header className="flex items-center justify-between border-b border-border px-6 py-4">
              <div>
                <h2 className="text-lg font-semibold text-text-primary">Test workflow</h2>
                <p className="text-sm text-text-secondary">{workflowName ?? "Unnamed workflow"}</p>
              </div>
            ...
            <main className="flex flex-1 overflow-hidden bg-background-secondary/30">
              {!workflowVersionId ? (
                <div className="flex h-full w-full flex-col items-center justify-center gap-3 text-sm text-text-secondary">
                  <p>Select or create a workflow version before starting a test session.</p>
                  <Button onClick={onClose} variant="outline">Close</Button>
                </div>
              ) : status === "starting" ? (
                <div className="flex h-full w-full items-center justify-center">
                  <LoadingSpinner message="Connecting to LiveKit…" size="lg" />
                </div>
              ) : error ? (
                <div className="flex h-full w-full flex-col items-center justify-center gap-4 text-center">
                  <div className="space-y-2">
                    <p className="text-base font-medium text-text-primary">Unable to start test session</p>
                ...
                <div className="flex h-full w-full items-center justify-center">
                  <LoadingSpinner message="Preparing session…" size="md" />
                </div>
              )}
            </main>
          </div>
