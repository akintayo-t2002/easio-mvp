...
export interface PathVariableUpdatePayload {
  name?: string
  description?: string | null
  data_type?: string
}
...
export function parseToolResponse(apiTool: AgentToolResponse): ToolOperation {
  ...
  const llmDescriptionValue = typeof configRaw.llmDescription === "string" ? configRaw.llmDescription : ""
...
export async function updatePath(pathId: string, payload: PathUpdatePayload) {
  return request<AgentPathResponse>(`/paths/${pathId}`, {
    method: "PATCH",
    body: JSON.stringify(payload),
  })
}

export async function updatePathVariable(variableId: string, payload: PathVariableUpdatePayload) {
  return request<PathVariableResponse>(`/path-variables/${variableId}`, {
    method: "PATCH",
    body: JSON.stringify(payload),
  })
}

export async function deletePathVariable(variableId: string) {
  await request<void>(`/path-variables/${variableId}`, {
    method: "DELETE",
  })
}
...
export async function updateWorkflowVersionConfig(versionId: string, config: Record<string, unknown>) {
